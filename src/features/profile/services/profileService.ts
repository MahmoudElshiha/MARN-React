...
function validateLegalRequiredFields(data: ProfileSettings) {
  ...
  if (missing.length > 0) {
    throw new Error(
      `Please complete required verification fields: ${missing.join(', ')}`,
    )
  }
}
...
export const profileService = {
  ...
  async getProfileSettings(): Promise<ProfileSettings> {
    const response = await apiClient.get<
      MaybeApiResponse<ProfileSettingsApiModel>
  ...
  async updateProfileSettings(
    ...
    return {
      ...data,
      profileImageUrl:
        data.profileImageUrl || (avatarFile ? avatarFile.name : ''),
    }
  },
  ...
  async updateProfileLegalSettings(
    ...
    return {
      ...data,
      frontIdPhotoUrl:
        data.frontIdPhotoUrl ||
        (files.frontIdCard ? files.frontIdCard.name : ''),
      backIdPhotoUrl:
